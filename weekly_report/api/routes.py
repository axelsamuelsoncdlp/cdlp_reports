...
@app.post("/api/generate/weekly-reports-pdf")
...
async def get_top_markets(
    ...
    try:
        ...
        # Fallback: Calculate top markets
        config = load_config(week=base_week)
        markets_data = calculate_top_markets_for_weeks(base_week, num_weeks, config.data_root)
...
async def get_budget_general(week: str = Query(...)):
    ...
    try:
        ...
        # Normalize columns and key values
        df.columns = df.columns.str.strip()
        ...
        # Exclude pre-aggregated total rows to avoid double counting
        if "Market" in df.columns:
            total_aliases = {"total", "all", "all markets", "grand total", "totals"}
            df = df[~df["Market"].str.lower().isin(total_aliases)]
            # Drop empty/na Market rows
            df = df[df["Market"].str.len() > 0]
            # Do not deduplicate by Month+Market here; lines represent distinct components

        # Identify dimension columns to exclude from numeric aggregation
        ...
        # Clean and coerce numeric-like columns with robust parser
        ...
        # Derive Gross Revenue from Net Revenue and Returns when available to ensure consistency
        def derive_gross(df: pd.DataFrame, net_col: str, returns_col: str, gross_col: str) -> None:
            if net_col in df.columns and returns_col in df.columns:
                try:
                    df[gross_col] = df[net_col] - df[returns_col]
                except Exception:
                    pass
        ...
        # Ensure Month is present for grouping; if missing, return error
        if "Month" not in df.columns:
            raise HTTPException(status_code=400, detail="Budget file missing 'Month' column")

        df_grouped = pd.concat([df[["Month"]], numeric_df], axis=1).groupby("Month", as_index=False).sum(numeric_only=True)

        # Sort months chronologically (expects format like 'April 2025')
        try:
        ...
        # Compute YTD totals up to today's month
        now = datetime.now()
        ...
        # Build metrics list from numeric columns and whitelist to avoid helper fields
        ...
        whitelist = [
            ...
            # Totals (keep in case needed downstream)
        ...
        # Desired priority order (case-insensitive exact match)
        desired_order = [
            # Returning section
            "Returning Customers",
            "Share of Returning Customers",
            "Returning Gross Revenue",
            "Returning Returns",
            "Returning Net Revenue",
            "Returning Cost of Goods Sold",
            "Returning Orders",
            "Returning Order Frequency",
            "Returning AOV",
        ...
        def metric_key(name: str):
            ...
            # Primary: defined order; Secondary: group by customer tag to keep consistency; Tertiary: alphabetical
            return (0, idx) if idx is not None else (1, name.lower())
        metric_cols = sorted(metric_cols, key=metric_key)

        # Build pivot data: metrics -> month -> value, plus total per metric
        table = {}
        totals = {}
        ytd_totals = {}
        customer_by_metric = {}
        ...
        for metric in metric_cols:
            # Classify customer segment based on metric name
            ...
            # Normalize display name for share metrics
            ...
            # YTD sum (months <= current month of current year)
            ytd_val = 0.0
