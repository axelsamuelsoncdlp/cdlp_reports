...
export default function BatchFileUpload({
    ...
    try {
      ...
      if (onProgress) {
        progressInterval = setInterval(() => {
          ...
          // Estimera progress baserat på tid (konservativ uppskattning)
          // Större filer tar längre tid, så vi justerar baserat på filstorlek
          const estimatedTime = Math.max(2000, fileSize / 10000) // Minst 2 sekunder, annars baserat på storlek
      ...
      // Använd fetch med timeout
      const controller = new AbortController()
      timeoutId = setTimeout(() => controller.abort(), 120000) // 120 sekunder timeout
  ...
  const handleUploadAll = async () => {
    ...
    for (let i = 0; i < filesToUpload.length; i++) {
      ...
      const result = await uploadSingleFile(type, file, (progress) => {
        setCurrentUploadingFile(prev => prev ? { ...prev, progress } : null)
      })
                ...
                <>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-900 truncate">
                      Uploading: {currentUploadingFile.label}
                    </div>
