...
export default function ContributionNewPerCountry() {
  ...
  // Normalize contribution_new_per_country structure - handle both { contribution_new_per_country: [...] } and direct array
  let contributionData: any[] = []
  if (contribution_new_per_country) {
    if (Array.isArray(contribution_new_per_country)) {
      // Structure: direct array
      contributionData = contribution_new_per_country
    } else if (contribution_new_per_country.contribution_new_per_country && Array.isArray(contribution_new_per_country.contribution_new_per_country)) {
      // Structure: { contribution_new_per_country: [...] }
      contributionData = contribution_new_per_country.contribution_new_per_country
    } else if (typeof contribution_new_per_country === 'object') {
      // Structure: { contribution_new_per_country: {...} } - might be an object instead of array
      contributionData = Object.values(contribution_new_per_country.contribution_new_per_country || {}) as any[]
    }
  }

  if (!contribution_new_per_country || contributionData.length === 0) {
    return (
