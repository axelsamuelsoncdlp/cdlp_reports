...
export default function NCACPerCountry() {
  ...
  // Normalize ncac_per_country structure - handle both { ncac_per_country: [...] } and direct array
  let ncacData: any[] = []
  if (ncac_per_country) {
    if (Array.isArray(ncac_per_country)) {
      // Structure: direct array
      ncacData = ncac_per_country
    } else if (ncac_per_country.ncac_per_country && Array.isArray(ncac_per_country.ncac_per_country)) {
      // Structure: { ncac_per_country: [...] }
      ncacData = ncac_per_country.ncac_per_country
    } else if (typeof ncac_per_country === 'object') {
      // Structure: { ncac_per_country: {...} } - might be an object instead of array
      ncacData = Object.values(ncac_per_country.ncac_per_country || {}) as any[]
    }
  }

  if (!ncac_per_country || ncacData.length === 0) {
    return (
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-3 gap-6">
        {countryOrder.map((country, index) => {
          const chartData = ncacData.map((week: any) => {
