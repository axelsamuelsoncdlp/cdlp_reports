...
async function loadBudgetGeneralFromAPI(baseWeek: string): Promise<{ budget?: any; actuals?: any }> {
  try {
    const [budgetRes, actualsRes] = await Promise.all([
      fetch(`http://localhost:8000/api/budget-general?week=${baseWeek}`),
      fetch(`http://localhost:8000/api/actuals-general?week=${baseWeek}`),
    ])
...
export async function loadWeeklyReportMetricsFromSupabase(
  ...
  try {
    // Check if Supabase is properly configured
    if (!supabase || !process.env.NEXT_PUBLIC_SUPABASE_URL) {
      console.debug('Supabase not configured, falling back to API')
      if (fallbackToApi) {
    ...
    if (error) {
      ...
      // Handle network errors (ERR_QUIC_PROTOCOL_ERROR, Failed to fetch, etc.)
